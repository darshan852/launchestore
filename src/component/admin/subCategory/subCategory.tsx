...
import { convertResponse } from "./subCategoryCommon"

const SubCategory = () => {
  const router = useRouter()
  const [subCategory, setSubCategory] = React.useState<SubCategoryDetail[]>([])
  ...
  const handleGetSubCategory = React.useCallback(async () => {
    try {
      const params = {
        search: debounceText,
        page: page,
      ...
      const response = await newClientInstance.getSubCategoryList({
        params: params,
      })
  ...
  }, [handleGetSubCategory, debounceText, page, rowsPerPage])

  const columns: readonly Column[] = [
    { id: "name", label: "Sub Category Name", minWidth: 170, type: "text" },
    {
      id: "category_name",
      label: "Category Name",
      minWidth: 170,
  ...
  const handleDeleteSubCategory = async (value: number[]) => {
    try {
      const params = {
        ids: value,
      }
      const response = await newClientInstance.deleteSubCategory({
        params: params,
      })
      if (response.data.success) {
  ...
  const action: CustomeCheckboxAction[] = [
    {
      ...
      icon: FaEdit,
      iconDirection: "right",
      onClick: (value: SubCategoryDetail) => {
        router.push(`/admin/sub-category/edit/${value.id}`)
        console.log("first")
      },
      color: "inherit",
      className:
      ...
      onClick: (value: SubCategoryDetail) => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            handleDeleteSubCategory([value.id])
          }
        })
      },
      className:
  ...
  const checkBoxAction: CustomeCheckboxAction[] = [
    {
      ...
      onClick: (value: SubCategoryDetail[]) => {
        Swal.fire({
          ...
          /* Read more about isConfirmed, isDenied below */
          if (result.isConfirmed) {
            handleDeleteSubCategory(value.map((v) => v.id))
          }
        })
      },
      color: "inherit",
  ...
  const customeAction: CustomeCheckboxAction = {
    ...
    iconDirection: "left",
    onClick: () => {
      router.push("/admin/sub-category/add")
    },
    variant: "contained",
    color: "Primary",
  ...
  return (
    <>
      <CustomeTabel
        columns={columns}
        rows={convertResponse(subCategory)}
        page={page}
        setPage={setPage}
